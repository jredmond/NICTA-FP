...
instance Bind f => Apply (StateT s f) where
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (StateT f) <*> (StateT a) = StateT (\s -> (\(g,t) -> (\(z,u) -> (g z, u)) <$> a t) =<< f s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' lstA = eval' (filtering (\a -> state' (\s -> (S.notMember a s, S.insert a s))) lstA) S.empty
...
distinctF lstA = 
  evalT (filtering (\a -> StateT (\s -> 
    if a > 100 then Empty else Full (S.notMember a s, S.insert a s))) lstA) S.empty
...
instance Bind (Logger l) where
  k =<< Logger l1 a = let Logger l2 b = k a in Logger (l1 ++ l2) b
...
distinctG listA = runOptionalT (evalT (filtering (\a -> StateT (\s -> 
  OptionalT (if a > 100 then 
              log1 (fromString ("aborting > 100: " P.++ show a)) Empty 
             else if even a then
              log1 (fromString ("even number: " P.++ show a)) (Full (S.notMember a s, S.insert a s))
             else
              pure (Full (S.notMember a s, S.insert a s))
            ))) listA) S.empty)
